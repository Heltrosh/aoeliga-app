...
tournaments.put('/:slug', requireUser, withTournamentBySlug, requirePermission('tournament.update'), async (c) => {
...
tournaments.put('/:slug/divisions/:divisionId', requireUser, withTournamentBySlug, requirePermission('division.manage'), async (c) => {
  const t = c.get('tournament')!;
  const divisionId = Number(c.req.param('divisionId'));
  const body = await c.req.json<{ name?: string; ruleset_id?: number | null }>();
...
tournaments.delete('/:slug/divisions/:divisionId', requireUser, withTournamentBySlug, requirePermission('division.manage'), async (c) => {
  const t = c.get('tournament')!;
  const divisionId = Number(c.req.param('divisionId'));
  await c.env.DB.prepare(`DELETE FROM divisions WHERE id = ? AND tournament_id = ?`).bind(divisionId, t.id).run();
  return c.json({ ok: true });
});
...
tournaments.put('/:slug/admins/:userId', requireUser, withTournamentBySlug, requirePermission('admin.assign'), async (c) => {
  const t = c.get('tournament')!;
...
tournaments.delete('/:slug/admins/:userId', requireUser, withTournamentBySlug, requirePermission('admin.assign'), async (c) => {
  const t = c.get('tournament')!;
  const userId = Number(c.req.param('userId'));
  await c.env.DB.prepare(`DELETE FROM tournament_admins WHERE tournament_id = ? AND user_id = ?`).bind(t.id, userId).run();
  return c.json({ ok: true });
});
...
tournaments.put('/:slug/streamers/:userId', requireUser, withTournamentBySlug, requirePermission('streamer.manage'), async (c) => {
  const t = c.get('tournament')!;
  const userId = Number(c.req.param('userId'));
...
tournaments.delete('/:slug/streamers/:userId', requireUser, withTournamentBySlug, requirePermission('streamer.assign'), async (c) => {
  const t = c.get('tournament')!;
  const userId = Number(c.req.param('userId'));
  await c.env.DB.prepare(`DELETE FROM tournament_streamers WHERE tournament_id = ? AND user_id = ?`).bind(t.id, userId).run();
  return c.json({ ok: true });
});
...
tournaments.post('/:slug/players', requireUser, withTournamentBySlug, requirePermission('player.register'), async (c) => {
  const t = c.get('tournament')!;
  ...
  if (!actingAsSelf) {
    const allowed = await (await import('../lib/permissions')).can(c, 'player.manage');
...
tournaments.put('/:slug/players/:playerId', requireUser, withTournamentBySlug, requirePermission('player.manage'), async (c) => {
  const t = c.get('tournament')!;
  const playerId = Number(c.req.param('playerId'));
...
tournaments.delete('/:slug/players/:playerId', requireUser, withTournamentBySlug, requirePermission('player.manage'), async (c) => {
  const t = c.get('tournament')!;
  const playerId = Number(c.req.param('playerId'));
  await c.env.DB.prepare(`DELETE FROM tournament_players WHERE id = ? AND tournament_id = ?`).bind(playerId, t.id).run();
  return c.json({ ok: true });
});
...
tournaments.put('/:slug/matches/:matchId', requireUser, withTournamentBySlug, async (c) => {
  const t = c.get('tournament')!;
  const matchId = Number(c.req.param('matchId'));
  ...
  const canManage = await (await import('../lib/permissions')).can(c, 'match.manage');
  const canScheduleOwn = await (await import('../lib/permissions')).can(c, 'match.schedule.own', { matchId });
  if (!canManage && !canScheduleOwn) return c.json({ error: 'Forbidden' }, 403);
...
tournaments.delete('/:slug/matches/:matchId', requireUser, withTournamentBySlug, requirePermission('match.manage'), async (c) => {
  const t = c.get('tournament')!;
  const matchId = Number(c.req.param('matchId'));
  await c.env.DB.prepare(`DELETE FROM matches WHERE id = ? AND tournament_id = ?`).bind(matchId, t.id).run();
  return c.json({ ok: true });
});
...
tournaments.post('/:slug/matches/:matchId/replays', requireUser, withTournamentBySlug, requirePermission('replay.upload.own', (c) => ({ matchId: Number(c.req.param('matchId')) })), async (c) => {
  const t = c.get('tournament')!;
  const user = c.get('user')!;
  const matchId = Number(c.req.param('matchId'));
